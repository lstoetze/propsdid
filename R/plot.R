...
#' @param estimates.label provide labels for the estimates as vector. These will be shown in the facets.
#' @param show.lambda Should lambda estimates be shown as a ribbion on the y-axis. Default is FALSE
...
synthdid_plot = function(estimates, treated.name = 'treated', control.name = 'synthetic control',
			 spaghetti.units = c(), spaghetti.matrices = NULL,
                         facet = NULL, facet.vertical = TRUE, lambda.comparable = !is.null(facet), overlay = 0,
			                   show.lambda = FALSE, estimates.label = NULL,
  ...
  if (is.null(names(estimates)) & is.null(estimates.label)) { names(estimates) = sprintf('estimate %d', 1:length(estimates))}
  if (!is.null(estimates.label) & !(length(estimates)== length(estimates.label))) { stop('provided estimates labels need to have the same length as the number of estimates') }
  if (!is.null(estimates.label) & length(estimates)== length(estimates.label)){names(estimates) = estimates.label}
  ...
  p = ggplot() +
    ...
    # Show lambda
  if(show.lambda) {
  p = p + geom_ribbon(aes(x = x, ymin = ymin, ymax = ymax, group = color, fill = color, alpha = .5 * diagram.alpha * show),  data = no.sc(conc$ribbons),
...
synthdid_units_plot = function(estimates, negligible.threshold = .001, negligible.alpha = .3, se.method='jackknife', units=NULL) {
  ...
  p + facet_wrap(. ~ estimator) + xlab('') + ylab('') + guides(shape = 'none') +
...
#' Plot a synthdid multi object
#' @param x The object to plot
#' @param ... Additional arguments (currently ignored).
#' @method plot synthdid_estimate
#' @export
plot.synthdid_estimate_multi = function(x, which_plot=NULL,...) {
  if (requireNamespace("ggplot2", quietly = TRUE)) {
    .ignore <- tryCatch(attachNamespace("ggplot2"), error = function(e) e)
  } else {
    stop("Plotting requires the package `ggplot2`. Install it to use this function.")
  ...
  x_list <- createEstimateList(x, k_plot = which_plot)

  synthdid_plot(x_list, ...)
}
