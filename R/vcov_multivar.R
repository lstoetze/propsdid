#' Calculate Variance-Covariance Matrix for a Fitted Model Object multivar
#'
#' Provides adapted variance estimates for the multivariate outcome based on the following three options
#' \itemize{
...
#'
#' @param object A synthdid multi model
...
#' @export
vcov.synthdid_estimate_multi = function(object,
                                  method = c("bootstrap", "jackknife", "placebo"),
                                  replications = 200, ...) {
  method = match.arg(method)
  if(method == 'bootstrap') {
    se = bootstrap_se_multi(object, replications)
  } else if(method == 'jackknife') {
    se = jackknife_se_multi(object)
  } else if(method == 'placebo') {
    se = placebo_se_multi(object, replications)
  }
  return(se^2)
}
...
bootstrap_se_multi = function(estimate, replications) { sqrt((replications-1)/replications) * apply(bootstrap_sample_multi(estimate, replications),2,sd) }
bootstrap_sample_multi = function(estimate, replications) {
  setup = attr(estimate, 'setup')
  opts = attr(estimate, 'opts')
  weights = attr(estimate, 'weights')
  if (setup$N0 == dim(setup$Y)[1] - 1) { return(NA) }
  theta = function(ind) {
    if(all(ind <= setup$N0) || all(ind > setup$N0)) { NA }
      ...
      do.call(synthdid_estimate_multivar,
              c(list(Y=setup$Y[sort(ind),,],
                     N0=sum(ind <= setup$N0),
                     T0=setup$T0,
                    # X=setup$X[sort(ind), ,],  No Covariates
                     weights=weights.boot), opts))
    }
  }
  bootstrap.estimates = matrix(NA, nrow=replications, ncol=setup$K)
  count = 0
  while(count < replications) {
    bootstrap.estimates[count+1,] = theta(sample(1:dim(setup$Y)[1], replace=TRUE))
...
jackknife_se_multi = function(estimate, weights = attr(estimate, 'weights')) {
  setup = attr(estimate, 'setup')
  opts = attr(estimate, 'opts')
  if (!is.null(weights)) {
    opts$update.omega = opts$update.lambda = FALSE
  }
  if (setup$N0 == dim(setup$Y)[1] - 1 || (!is.null(weights) && sum(weights$omega != 0) == 1)) { return(NA) }
  theta = function(ind) {
    weights.jk = weights
    if (!is.null(weights)) { weights.jk$omega = sum_normalize(weights$omega[ind[ind <= setup$N0]]) }
    estimate.jk = do.call(synthdid_estimate_multivar,
                          c(list(Y=setup$Y[ind, ,],
                                 N0=sum(ind <= setup$N0),
                                 T0=setup$T0,
                                 # X = setup$X[ind, , ],  No covariates
                                 weights = weights.jk), opts))
  }
  jackknife_multi(1:dim(setup$Y)[1], theta, setup$K)
}
...
#' @keywords internal
jackknife_multi = function(x, theta, K) {
  n = length(x)
  u = matrix(NA, nrow=n,ncol=K)
  for (i in 1:n) {
    u[i,] = theta(x[-i])
  }
  jack.se = sqrt(((n - 1) / n) * (n - 1) * apply(u,2,var))
...
placebo_se_multi = function(estimate, replications) {
  setup = attr(estimate, 'setup')
  opts = attr(estimate, 'opts')
  weights = attr(estimate, 'weights')
  N1 = dim(setup$Y)[1] - setup$N0
  ...
  theta = function(ind) {
    ...
    do.call(synthdid_estimate_multivar, c(list(Y=setup$Y[ind,,],
                                            N0=N0,
                                            T0=setup$T0,
                                            # X=setup$X[ind, ,],no covariates
                                            weights=weights.boot), opts))
